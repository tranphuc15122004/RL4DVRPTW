"""
PolyNet model — Single-instance inference from a CSV file.

Loads one CSV scenario, runs the PolyNet model, and prints detailed per-vehicle
routes, costs, constraint violations, and diagnostics.

Usage (all params explicit):
    python3 polynet/infer_single.py --data-csv data/datasets/100/h100c101.csv \\
...
Usage (auto-configure from training args.json):
    python3 polynet/infer_single.py --data-csv data/datasets/100/h100c101.csv \\
        --model-weight data/_PolyNet/chkpt_best.pyth \\
        --model-args data/_PolyNet/args.json
...
def main():
    ...
    # --- Initialize model ---
    learner = init_polynet_model(args, env_cls, device)
    learner.eval()
    poly_k = args.cust_k if args.cust_k is not None else (args.customers_count + 1)
    print(f"PolyNet model initialised (model_size={args.model_size}, "
          f"layers={args.layer_count}, heads={args.head_count}, k={max(2, int(poly_k))})")
