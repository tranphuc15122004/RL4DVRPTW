...
def main(args):
    ...
    learner : torch.Module = AM_DVRPTW(
        cust_feat_size = Environment.CUST_FEAT_SIZE,
        veh_state_size = Environment.VEH_STATE_SIZE,
    ...
    if args.baseline_type not in {"none", "critic"}:
        raise ValueError(
            "am.train currently supports --baseline-type in {'none', 'critic'} only"
        )
